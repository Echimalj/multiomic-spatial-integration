...
prepare_absolute_abundance_data <- function(df,
  ...
  abundance_values <- suppressWarnings(
    as.numeric(df[[abundance_col]])
  )

  offset_values <- suppressWarnings(
    as.numeric(df[[offset_col]])
  )

  if (any(!is.finite(abundance_values))) {
    stop(
  ...
  if (any(abundance_values <= 0)) {
    bad_n <- sum(abundance_values <= 0)

    stop(
      "prepare_absolute_abundance_data: '",
      abundance_col,
      "' must contain strictly positive values. Found ",
      bad_n,
  ...
  if (any(offset_values <= 0)) {
    bad_n <- sum(offset_values <= 0)
  ...
  df[[abundance_col]] <- abundance_values
  df[[offset_col]] <- offset_values

  df$log_abs_abundance <- log(abundance_values)
  df$log_offset <- log(offset_values)
...
.run_absolute_model <- function(tmp, formula, min_rows = 5, min_scans = 2) {
  ...
  if (dplyr::n_distinct(tmp$Scan_ID, na.rm = TRUE) < min_scans) {
    return(NULL)
  }
  ...
  fit <- tryCatch(
    ...
    error = function(e) {
      message(
        "Absolute-abundance model failed: ",
        conditionMessage(e)
      )
      NULL
    }
...
run_amyloid_effect_absolute <- function(df) {
  ...
  if (nrow(dat_ad) == 0) {
    stop(
      "run_amyloid_effect_absolute(): no AD-CAA observations were found.",
      call. = FALSE
    )
  }
  ...
  for (rg in levels(dat_ad$region)) {
    for (ct in levels(dat_ad$celltype)) {
      ...
      tmp <- dat_ad |>
        ...
        dplyr::filter(
          !is.na(.data$log_abs_abundance),
          is.finite(.data$log_abs_abundance),
          !is.na(.data$log_offset),
          is.finite(.data$log_offset)
        ) |>
        droplevels()

      n_pathology <- dplyr::n_distinct(
        tmp$pathology[!is.na(tmp$pathology)]
      )

      n_scans <- dplyr::n_distinct(
        tmp$Scan_ID[!is.na(tmp$Scan_ID)]
      )

      if (nrow(tmp) == 0) {
        fit_status[[result_key]] <- data.frame(
          region = rg,
          celltype = ct,
          n_obs = 0L,
          n_pathology = 0L,
      ...
      if (n_pathology < 2) {
        fit_status[[result_key]] <- data.frame(
          ...
          n_scans = n_scans,
          status = "fewer_than_two_pathology_levels"
        )
        next
      }
      ...
      mean_results[[result_key]] <- emm_df |>
        dplyr::mutate(
          celltype = ct,
          region = rg
        )
      ...
      contrast_results[[result_key]] <-
        as.data.frame(summary(cont, infer = TRUE)) |>
        dplyr::mutate(
          celltype = ct,
          region = rg,
          fold_ratio = exp(.data$estimate)
        )
  ...
  if (length(contrast_results) == 0) {
    status_summary <- fit_status_df |>
      dplyr::count(.data$status, name = "n_combinations")

    print(status_summary)

    stop(
      "run_amyloid_effect_absolute(): no valid amyloid contrasts were ",
      "produced. See the printed fit-status summary above.",
      call. = FALSE
    )
  }
